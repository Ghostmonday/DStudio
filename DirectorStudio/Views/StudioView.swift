...
struct StudioView: View {
    ...
    private func projectHeaderSection(project: ProjectModel) -> some View {
    ...
    private func segmentsSection(project: ProjectModel) -> some View {
...
struct SceneCardWithVideoGeneration: View {
    ...
    private func cinematicTagsView(tags: CinematicTags) -> some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ...
                if let movement = tags.cameraMovement {
                    Tag(text: movement, icon: "arrow.triangle.2.circlepath")
                }
            }
        }
    }
...
class CreditWallet: ObservableObject {
    @Published var balance: Int = 5 // Default balance
}

// MARK: - Placeholder Services/Models
...
struct PromptSegment: Identifiable {
    let id: UUID
    let index: Int
    let content: String
    let duration: Int
    let cinematicTags: CinematicTags?
}

struct CinematicTags {
    let shotType: String
    let lighting: String
...
struct ClaimIncludedClipSheet: View {
    ...
    var body: some View {
        NavigationView {
            VStack {
                Text("Claim your included clip")
                    .foregroundColor(.white)
            }
            .frame(maxWidth: .infinity, maxHeight: .infinity)
            .background(Color.black)
            .navigationTitle("Free Clip")
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("Cancel") { dismiss() }
                }
