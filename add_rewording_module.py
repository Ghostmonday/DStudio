...
# Generate UUIDs
rewording_uuid = str(uuid.uuid4()).replace('-', '').upper()[:24]
rewording_build_uuid = str(uuid.uuid4()).replace('-', '').upper()[:24]

# Add RewordingModule.swift file reference
file_ref = f"""
		1A0000{rewording_uuid[:6]} /* RewordingModule.swift */ = {{isa = PBXFileReference; lastKnownFileType = sourcecode.swift; path = "DirectorStudio/Modules/RewordingModule.swift"; sourceTree = "<group>"; }};"""
...
modules_replacement = r'\1\n\t\t\t1A0000' + rewording_uuid[:6] + ' /* RewordingModule.swift */,'

# Add to build file
build_file = f"""
		1A0000{rewording_build_uuid[:6]} /* RewordingModule.swift in Sources */ = {{isa = PBXBuildFile; fileRef = 1A0000{rewording_uuid[:6]} /* RewordingModule.swift */; }};"""
...
sources_replacement = r'\1\n\t\t\t1A0000' + rewording_build_uuid[:6] + ' /* RewordingModule.swift in Sources */,'
